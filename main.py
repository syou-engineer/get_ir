...
if __name__ == '__main__':
    ...
    for url in url_list:
        ...
        for idx, data in enumerate(df_new):
            ...
            if item:
                if not item["body"] == body:
                    is_update = True
                    print(f"{company_name}：更新されました。{body}")
                else:
                    print(f"{company_name}：IR情報は更新されておりません")
                    break
        ...
        put_item_to_daynamodb(dynamodb, df_new.iloc[0, 0], df_new.iloc[0, 2])
